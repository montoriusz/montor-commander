...
pub fn resolve_model(providers: &[Provider], alias: Option<&str>) -> Result<ResolvedModel, String> {
    let from_entry =
    ...
    // No selection: pick a default.
    let primary = providers
        .iter()
        .find(|p| p.enabled && p.is_primary && !p.models.is_empty());
    let pick = primary.or_else(|| {
        providers.iter().find(|p| {
            p.enabled && !p.is_primary && !p.alias.trim().is_empty() && !p.models.is_empty()
        })
    });
    let Some(p) = pick else {
        return Err("No model configured — add one in Settings.".to_string());
    };
    let m = p.models.first().unwrap();
    let real_name = m.name.clone();
    let alias_out = if p.is_primary {
        real_name.clone()
    } else {
        format!("{}::{}", p.alias.trim(), real_name)
    };
    Ok(from_entry(p, m, real_name, alias_out))
}
...
pub async fn list_available_models(
    ...
    for p in providers {
        ...
        let model_names: Vec<String> = match p.kind {
            AdapterKind::Ollama => {
                match fetch_model_names(p.kind, p.base_url.as_deref(), key).await {
                    Ok(names) => names,
                    Err(e) => {
                        tracing::warn!(
                            provider_id = %p.id,
...
/// Live-fetch the model names for one provider endpoint via `genai`'s
/// `all_model_names`. `base_url` overrides the adapter default; an empty/absent
/// key falls back to genai's env-var lookup for the adapter.
...
mod tests {
    ...
    fn resolve_model_primary_bare_alias() {
        ...
        let r = resolve_model(&providers, Some("gpt-4o")).unwrap();
    ...
    fn resolve_model_non_primary_namespaced_alias() {
        ...
        let r = resolve_model(&providers, Some("myrouter::openai/gpt-4o")).unwrap();
    ...
    fn resolve_model_strips_prefix_in_real_name() {
        ...
        let r = resolve_model(&providers, Some("r::m1")).unwrap();
    ...
    fn resolve_model_uses_keychain_key_when_present() {
        ...
        let r = resolve_model(&[prov], Some("gpt-4o")).unwrap();
    ...
    fn resolve_model_skips_disabled_providers() {
        ...
        let err = resolve_model(&providers, Some("gpt-4o")).unwrap_err();
    ...
    fn resolve_model_unresolvable_alias_errors() {
        ...
        let err = resolve_model(&providers, Some("grok-99")).unwrap_err();
        assert!(err.contains("No provider/model matches"));
    }

    #[test]
    fn resolve_model_default_picks_primary_first_model() {
        let providers = vec![
            aliased("r", "R", "r", AdapterKind::OpenAI, "https://x", &["m1"]),
            primary("gemini", AdapterKind::Gemini, &["gemini-pro-latest"]),
        ];
        let r = resolve_model(&providers, None).unwrap();
        assert_eq!(r.alias, "gemini-pro-latest");
        assert_eq!(r.kind, AdapterKind::Gemini);
    }

    #[test]
    fn resolve_model_default_falls_back_to_first_addressable_non_primary() {
        // No primary provider with models — pick the first non-primary whose
        // alias makes it addressable.
        let providers = vec![
            aliased("a", "A", "", AdapterKind::OpenAI, "https://x", &["m1"]), // no alias
            aliased("b", "B", "bb", AdapterKind::OpenAI, "https://y", &["m1"]), // addressable
        ];
        let r = resolve_model(&providers, None).unwrap();
        assert_eq!(r.alias, "bb::m1");
        assert_eq!(r.model_name, "m1");
    }

    #[test]
    fn resolve_model_default_errors_when_nothing_configured() {
        let err = resolve_model(&[], None).unwrap_err();
